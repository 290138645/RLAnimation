...
-(id)initWithLayer:(RLCircleLayer *)layer{
...
- (void)drawInContext:(CGContextRef)ctx{
    CGFloat x      = self.outsideRect.origin.x;    // x坐标
    CGFloat y      = self.outsideRect.origin.y;    // y坐标
    CGFloat width  = self.outsideRect.size.width;  // 宽度
    CGFloat height = self.outsideRect.size.height; // 高度
    ...
    CGFloat movedDistance = (width * 1 / 6) * fabs(self.progress - 0.5) * 2;
    
    //方便下方计算各点坐标，先算出外接矩形的中心点坐标
    CGPoint rectCenter = CGPointMake(x + width / 2, y + height / 2);
    
    //ABCD 中心点
    CGPoint pointA = CGPointMake(rectCenter.x ,y + movedDistance);
    CGPoint pointB = CGPointMake(self.movePoint == POINT_D ? rectCenter.x + width /2 : rectCenter.x + width / 2 + movedDistance * 2 ,rectCenter.y);
    CGPoint pointC = CGPointMake(rectCenter.x ,rectCenter.y + height / 2 - movedDistance);
    CGPoint pointD = CGPointMake(self.movePoint == POINT_D ? x - movedDistance*2 : x, rectCenter.y);
